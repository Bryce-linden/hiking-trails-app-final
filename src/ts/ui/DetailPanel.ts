...
export default class SelectionPanel {
  ...
  constructor(trails, state: State) {
    ...
    state.watch("selectedTrailId", (id) => {
      ...
      if (id) {
        //the const trail doesn't work here because selectedTrail is still undefined
  ...
  displayAppInfo() {
    if (this.state.device === "mobilePortrait") {
      this.detailInfograph.innerHTML = "This app shows the hikes in Dali Cangshan National Geopark. Select a hike on the map to find out more about it.";
    } else {
  ...
  displayInfo(trail: Trail): void {
    ...
    this.detailDescription.innerHTML = `<b>Explanation: </b> ${ trail.description }`;
  }
