...
// import * as Camera from "@arcgis/core/Camera";
...
export default class SceneElement {
      ...
      this.view.hitTest(event, {include: this.trailsLayer})
        .then((response) => {
          ...
          if (result && result.graphic) {
              console.log("Selected trail is not null", result.graphic.attributes[config.data.trailAttributes.id]);
              this.state.setSelectedTrail(result.graphic.attributes[config.data.trailAttributes.id]);
          } else {
            console.log("the selected trail is null");
  ...
  private selectFeature(featureId): void {
    ...
    var featureidmine = featureId
    const renderer = (<UniqueValueRenderer> this.trailsLayer.renderer).clone();
    console.log(this.trailsLayer.renderer);
    renderer.uniqueValueInfos = getUniqueValueInfos({ selection: featureId });
    this.trailsLayer.renderer = renderer;
    console.log(renderer.uniqueValueInfos);

    // this.trailsLayer.labelingInfo = getLabelingInfo({ selection: featureId });
    // this.state.setSelectedTrail(featureId)
    console.log("test select feature!!! The problem is right below")

    console.log(this.state.selectedTrail.geometry)
    // console.log(id)
    
//  { target: this.state.trails, tilt: 60 },
    this.view.goTo(
